...
def evaluate(args, model, tokenizer, labels, pad_token_label_id, best, mode, prefix="", verbose=True):
    ...
    correct_preds_ls, total_correct_ls, total_preds_ls = [0]*19, [0]*19, [0]*19 ##MODIFICATION!!!
    for ground_truth_id,predicted_id in zip(out_id_list,preds_id_list):
        ...
        # Updating the i variables MODIFICATION!!!
        correct_preds_ls[ground_truth_id] += len(lab_chunks & lab_pred_chunks) #MODIFICATION!!!
        total_correct_ls[predicted_id]   += len(lab_pred_chunks) #MODIFICATION!!!
        total_preds_ls[ground_truth_id] += len(lab_chunks) #MODIFICATION!!!

    p_ls = np.array(correct_preds_ls) / np.array(total_preds_ls) #MODIFICATION!!!
    r_ls = np.array(correct_preds_ls) / np.array(total_correct) #MODIFICATION!!!
    p = np.average(p_ls) #MODIFICATION!!!
    r = np.average(r_ls) #MODIFICATION!!!
